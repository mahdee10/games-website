...
export default function GamePoster({ src, title, published, path}) {
    ...
    return (
        <div
            ...
            {!hidden && published && <Link to={path} className="border-2 border-white font-bold md:px-3 md:py-2 py-1 px-1 md:text-base text-sm rounded absolute top-[73%] left-1/2 z-10 text-white transform -translate-x-1/2 -translate-y-1/2 hover:bg-[red]">{t("game-play")}</Link>}
